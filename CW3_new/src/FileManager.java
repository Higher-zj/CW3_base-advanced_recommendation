            ...
            while ((line = reader.readLine()) != null) {
                try {
                    ...
                    if (parts.length >= 4) {
                        ...
                        if (!parts[3].trim().isEmpty()) {
                            ...
                            for (String item : historyItems) {
                                if (!item.trim().isEmpty()) {
                                    ...
                                    String[] recordParts = item.split("@");
                                    if (recordParts.length >= 2) {
                                        String movieId = recordParts[0].trim();
                                        String date = recordParts[1].trim();
                                        history.addMovie(movieId, date);
                                    }
                        ...
                        // Remove movies from the watchlist that have already appeared in the viewing history
                        Set<String> historyMovieIds = history.getUniqueMovieIds();
                        List<String> watchlistMovieIds = watchlist.getMovieIds();
                        for (String movieId : historyMovieIds) {
    ...
    public static void saveUsers(Map<String, User> users) {
        try (PrintWriter writer = new PrintWriter(new FileWriter(USER_FILE))) {
            ...
            for (User user : users.values()) {
                ...
                String historyStr = "";
                List<String> historyRecords = user.getHistory().getMovieRecords();
                for (int i = 0; i < historyRecords.size(); i++) {
                    if (i > 0) historyStr += ";";
                    historyStr += historyRecords.get(i);
                }
